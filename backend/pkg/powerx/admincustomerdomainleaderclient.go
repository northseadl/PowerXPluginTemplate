...
func (c *AdminCustomerdomainLeader) GetLead(req *types.GetLeadReqeuest) (*types.GetLeadReply, error) {
	...
	err := c.H.Df().Method(http.MethodGet).
		Uri(fmt.Sprintf("/api/v1/admin/customerdomain/leads/%s", req.Id)).
		BindQuery(req).
		Result(res)
	if err != nil {
		return nil, err
...
func (c *AdminCustomerdomainLeader) PutLead(req *types.PutLeadRequest) (*types.PutLeadReply, error) {
	...
	err := c.H.Df().Method(http.MethodPut).
		Uri(fmt.Sprintf("/api/v1/admin/customerdomain/leads/%s", req.LeadId)).
		Json(req).
		Result(res)
	if err != nil {
		return nil, err
...
func (c *AdminCustomerdomainLeader) AssignLeadToEmployee(req *types.AssignLeadToEmployeeRequest) (*types.AssignLeadToEmployeeReply, error) {
	...
	err := c.H.Df().Method(http.MethodPost).
		Uri(fmt.Sprintf("/api/v1/admin/customerdomain/leads/%s/actions/employees", req.Id)).
		Json(req).
		Result(res)
	if err != nil {
		return nil, err
